...
# ============================================================
st.markdown('<div class="main-title fade">🤖 AI & ML Premium Project Hub</div>', unsafe_allow_html=True)
st.markdown('<div class="sub fade">50+ Industry-Grade Projects • Developed by <b>Ankit Mishra</b></div>', unsafe_allow_html=True)

# ============================================================
# LOAD PROJECT FILES FROM /PROJECT
# ============================================================

PROJECTS = {}
project_folder = "PROJECT"   # 🔥 YOU USE THIS FOLDER

# Make sure folder exists
if not os.path.exists(project_folder):
    os.makedirs(project_folder)

# Make sure __init__.py exists (required for import)
init_path = os.path.join(project_folder, "__init__.py")
if not os.path.exists(init_path):
    open(init_path, "w").close()

# Load project dictionaries
for file in os.listdir(project_folder):
    if file.endswith(".py") and file != "__init__.py":
        try:
            module_name = file.replace(".py", "")
            module = importlib.import_module(f"{project_folder}.{module_name}")
            ...
            if hasattr(module, "PROJECT"):
                ...
                PROJECTS[category][proj["name"]] = proj
...
with st.sidebar:
    ...
    project_names = list(PROJECTS[category].keys())

    if search:
        project_names = [p for p in project_names if search.lower() in p.lower()]

    selected_project = st.selectbox("Select Project", project_names)

    st.markdown("---")
    st.info("📁 Add/Remove projects by placing `.py` files inside the `PROJECT/` folder.")
...
st.download_button(
    ...
    data=create_zip(),
    file_name="All_AI_ML_Projects_By_Ankit_Mishra.zip",
    mime="application/zip"
)
...
# ============================================================
project = PROJECTS[category][selected_project]
...
with tab1:
    ...
    st.markdown("### 📦 Dataset")
    st.info(project.get("dataset", "Dataset not provided."))
...
# ============================================================
# TAB 3: CODE (SHOW + COPY + DOWNLOAD)
# ============================================================
with tab3:
    st.markdown("### 💻 Complete Project Code")

    code_text = project.get("code", "# No code available")

    # Copy-friendly text input
    st.text_input("Click & press CTRL + C to copy the code:", value=code_text, key="copy_box")

    # Download button
    st.download_button(
        "📥 Download Code File",
        data=code_text,
        file_name=f"{project['name'].replace(' ','_')}.py",
        mime="text/plain"
    )
...
# ============================================================
with tab4:
    st.markdown("### 📁 Files in PROJECT Folder")
    st.write(os.listdir(project_folder))
...
<hr>
<div style='text-align:center; color:#888; font-size:16px;'>
🚀 Built with ❤️ by <b>Ankit Mishra</b><br>
AI/ML Project Hub • Streamlit Premium Edition
</div>
""", unsafe_allow_html=True)
